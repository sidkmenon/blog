...
describe('remarkFootnotes', () => {
  ...
  it('handles comma-separated footnote references', () => {
    ...
    const html = (htmlNode as any).children.find((c: any) => c.type === 'html');
  ...
  it('handles subsequent references to the same footnote', () => {
    ...
    const firstPara = result.children[0] as any;
    const firstHtml = firstPara.children.find((c: any) => c.type === 'html');
    ...
    const secondPara = result.children[1] as any;
    const secondHtml = secondPara.children.find((c: any) => c.type === 'html');
